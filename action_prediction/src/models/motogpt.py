...
class MotoGPT(nn.Module):
    def __init__(
        ...
        self.learnable_latent_motion_embedding = torch.nn.Parameter(torch.zeros(1, num_latent_motion, self.hidden_size))
        nn.init.trunc_normal_(self.learnable_latent_motion_embedding, std=0.02)
        ...
        self.motion_prediction_head = MLP(self.hidden_size, self.hidden_size, latent_motion_codebook_size)
        
    def forward(self, image, language, language_attention_mask):
        ...
        condition_input_embeddings = torch.cat([lang_embeddings, image_embeddings], dim=1)
        ...
        latent_motion_embeddings = self.query_head(self.learnable_latent_motion_embedding, hidden_states)
        predicted_latent_motion_logits = self.motion_prediction_head(latent_motion_embeddings)
        # print("the predicted_latent_motion_logits is: ", predicted_latent_motion_logits.shape)
        return predicted_latent_motion_logits
    ...
    def compute_loss(self, image, batch):
        ...
        predicted_latent_motion_logits = self(image, language, language_attention_mask)
        # print("the predicted_latent_motion_logits is: ", predicted_latent_motion_logits.shape)
        # print("the gt_latent_motion_ids is: ", gt_latent_motion_ids.shape)
        gt_latent_motion_ids = gt_latent_motion_ids.squeeze(1)
        B, N, D = predicted_latent_motion_logits.shape
        predicted_latent_motion_logits = predicted_latent_motion_logits.reshape(B*N, D)
        gt_latent_motion_ids = gt_latent_motion_ids.reshape(B*N)
        loss = F.cross_entropy(predicted_latent_motion_logits, gt_latent_motion_ids)
        return {
            "loss": loss,
        }
