...
from action_prediction.src.trainers.optimizer import get_optimizer, LinearWarmup_CosineAnnealing
from contextlib import contextmanager
import cv2
from tqdm import tqdm
class LatentMotionPredictionTrainer:
    def __init__(
        self,
        motion_prediction_transformer,
        rgb_preprocessor,
        train_dataloader,
        eval_dataloader,
        save_path,
        ...
        if resume_ckpt_path is not None:
            ...
            missing_keys, unexpected_keys = motion_prediction_transformer.load_state_dict(torch.load(os.path.join(resume_ckpt_path, 'pytorch_model.bin'), map_location='cpu'), strict=False)
        ...
        optimizer = get_optimizer(
                        [p for n, p in motion_prediction_transformer.named_parameters() if p.requires_grad], 
                        lr=lr_max, 
                        wd=weight_decay
                    )
        ...
        motion_prediction_transformer, optimizer, train_dataloader, eval_dataloader = accelerator.prepare(
            motion_prediction_transformer, optimizer, train_dataloader, eval_dataloader, 
        ...
        self.motion_prediction_transformer = motion_prediction_transformer
        self.optimizer = optimizer
    ...
    def save_checkpoint(self, save_dir):
        unwrapped_motion_prediction_transformer = self.accelerator.unwrap_model(self.motion_prediction_transformer)
        state_dict = unwrapped_motion_prediction_transformer.get_state_dict_to_save()
        
        torch.save(state_dict, os.path.join(save_dir, "pytorch_model.bin"))
        omegaconf.OmegaConf.save(unwrapped_motion_prediction_transformer.config, os.path.join(save_dir, "config.yaml"))
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs+1):
            ...
            # 重新初始化prefetcher
            self.train_prefetcher = DataPrefetcher(self.train_prefetcher.loader, self.device, lang_tokenizer=self.lang_tokenizer)
            batch, load_time = self.train_prefetcher.next()
            ...
            while batch is not None:
                with self.accelerator.accumulate(self.motion_prediction_transformer):

                    self.motion_prediction_transformer.train()
                ...
                #         self.motion_prediction_transformer.eval()
                #         loss = self.calculate_loss(batch, train=True)
    ...
    def calculate_loss(self, batch, train):
        # image preprocessing
        # import pdb; pdb.set_trace()
        rgb_seq = batch[self.obs_name].unsqueeze(1)
        rgb_seq = self.rgb_preprocessor(rgb_seq, train=train)
        rgb_seq = rgb_seq.squeeze(1)
        # compute loss
        # print("the batch['task'] is: ", batch['task'])
        loss = self.motion_prediction_transformer.compute_loss(
            image=rgb_seq,
            batch=batch
        )
