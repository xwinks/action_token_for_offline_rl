...
class ActionPredictionTrainer:
    def __init__(
        ...
        if resume_ckpt_path is not None:
            ...
            missing_keys, unexpected_keys = action_prediction_transformer.load_state_dict(torch.load(os.path.join(resume_ckpt_path, 'pytorch_model.bin'), map_location='cpu'), strict=False)
        ...
        optimizer = get_optimizer(
                        [p for n, p in action_prediction_transformer.named_parameters() if p.requires_grad], 
                        lr=lr_max, 
                        wd=weight_decay
                    )
        ...
        action_prediction_transformer, optimizer, train_dataloader, eval_dataloader = accelerator.prepare(
            action_prediction_transformer, optimizer, train_dataloader, eval_dataloader, 
        ...
        self.action_prediction_transformer = action_prediction_transformer
        self.optimizer = optimizer
    ...
    def save_checkpoint(self, save_dir):
        unwrapped_action_prediction_transformer = self.accelerator.unwrap_model(self.action_prediction_transformer)
        state_dict = unwrapped_action_prediction_transformer.get_state_dict_to_save()
        
        torch.save(state_dict, os.path.join(save_dir, "pytorch_model.bin"))
        omegaconf.OmegaConf.save(unwrapped_action_prediction_transformer.config, os.path.join(save_dir, "config.yaml"))
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs+1):
            ...
            while batch is not None:
                with self.accelerator.accumulate(self.action_prediction_transformer):

                    self.action_prediction_transformer.train()
                ...
                #         self.action_prediction_transformer.eval()
                #         loss = self.calculate_loss(batch, train=True)
    ...
    def calculate_loss(self, batch, train):
        ...
        loss = self.action_prediction_transformer.compute_loss(
            image=rgb_seq,
            batch=batch
        )
